/* =====================================================
   めいさんクエスト(仮) マスタデータ v0.2
   ここを書き換える/追記するだけでコンテンツが増える。
   将来は Google Sheets → このJSON形式に書き出す運用を想定。
   v0.2: そざいごとの入手方法(gather)と固有お世話イベント(events)を追加
   ===================================================== */
window.GAME_DATA = {
  ...
  /* ---------- そざいマスタ(Tier1) v0.3 ----------
     gather.type:
       infra  = 開拓すると施設が建ち、時間でストックが貯まる(回収のみ・ゲームなし)
       plant  = たねをまく → リアル時間で成長 → 収穫アクションゲーム
       timing / dig = 待ちなしの、その場ミニゲーム
     plant.growSec: 成長にかかる秒数(作物5〜10分、木もの20〜30分)
     harvest.engine: pluck(ぽんぽん摘み) / dig(ほりあて)
     care: 成長中に1回わく「おせわチャンス」(タップ撃退。やると★ボーナス) */
  materials: [
  ...
  /* ---------- クイズバンク v0.4 ----------
     ...
     type "shape"=県のかたち / "position"=ちずで ひかる いち */
  quizzes: [
    /* --- 開拓: かたち・いち --- */
