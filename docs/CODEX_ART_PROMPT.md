...
- `src/` の コードを 変更する(絵の 差しかえ処理は もう あります。SVG を 置くだけで 出ます)
- `reference/` を 触る(参照専用)
...
| 7 | どうぐ・しんの めいさん・マスコットの かたち **15個**(下の 第7回の 節を 見る) | `codex/art-tools-shin` ✅納品ずみ(2026-08-07。renkon は 既存 lotus に ゆずり 14個を 採用) |
...
### 第7回(どうぐ・しんの めいさん・マスコット)だけ 変わる ところ

この 15個の うち 上の 11個は まだ `ART_ASSET_LIST.md` に **のって いない**
あたらしい かたちです(アプリは いま 仮の 絵 ― かま=knife、くまで=fan など ―
を つかって います)。納品されたら データの アイコン名を こちらで 差しかえます。
下の 4個(chick 3種と bee)は リストに のって いる かたちですが、
発注もれで **まだ コード描画の まま**です。ファイル名 どおりの SVG を 置けば
そのまま 差しかわります。
...
| `chick` | たんけんヒヨコ ぴっけ(**マスコット**)。まるい きいろの 体、オレンジの くちばし、あたまに 毛が 1本 ぴょこん。ヘッダー・ストーリーに いつも 出る 主役 | ―(コード描画) |
| `chick-cheer` | ばんざいの ぴっけ。りょうての つばさを 上げ、目を とじて わらう。あたまの 毛が はねる(おいわい用) | ―(コード描画) |
| `chick-egg` | たまごの からから 顔を 出す ぴっけ(あたらしく はじまる しるし) | ―(コード描画) |
| `bee` | はち。しましまの おなか+2まいの はね。こわくない かお(はらう あそびの 的なので にくめない 感じ) | ―(コード描画) |

- どうぐ・たべもの・momiji の いろは ぜんぶ **c 1色**あつかいで よい
  (#MAIN 置きかえで 何色にも なる 形に する)
- **chick 3種は 同じ キャラに 見える ことが 最優先**。3まい ならべて 顔・体型・色を
  そろえる こと。マスコットは 色を 直に 書いて よい(#MAIN に しなくて よい)が、
  `npm run art:check` の 色衝突警告が 出ない こと
- 大きさの 目やす: どうぐ5個は 20〜34px、たべもの5個は 28〜34px、momiji は 16〜22px、
  chick は 28〜48px、bee は 22〜30px
