...
### 数のノルマ(全県ぴったり同じ。テストが強制する)

| 項目 | 数 |
|---|---|
| そざい | **5**(うち「その県だけのそざい」が1つ以上) |
| tier2 レシピ | **3** |
| tier3 レシピ | **2** |
| おまつり(tier4) | **1**(ゲーム種別は全県ユニーク) |
| あそびの種類 | **4以上**(そざいの収穫ゲームの種類) |

- 多すぎるのも だめ(先に作った県だけ豪華になり、あとの県が すかすかに見える)
- みず(いど)のような 共通そざいを 全県に置かない。**みずは地方に1県だけ**(めいすいのさと)
- ノルマを変えたいときは `gameData.test.ts` の `QUOTA` を直し、**全県ぶん**足す/削る
